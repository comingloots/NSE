...
def update_channel(row_id, chat_id, title, join_url):
    with closing(db()) as conn:
        conn.execute("UPDATE channels SET chat_id=?, title=?, join_url=? WHERE id=?", (chat_id, title, join_url, row_id))
        conn.commit()
...
async def channel_manager(callback: CallbackQuery):
    if not is_admin(callback.from_user.id): await callback.answer("Not authorized.",show_alert=True); return
    rows=[[InlineKeyboardButton(text=f"✏️ {r[2]}",callback_data=f"cedit:{r[0]}"),InlineKeyboardButton(text="🗑️",callback_data=f"cdel:{r[0]}")] for r in channels()]
...
async def channel_edit_start(callback: CallbackQuery):
    if not is_admin(callback.from_user.id): return
    rid=int(callback.data.split(":")[1])
    row=next((r for r in channels() if r[0]==rid), None)
    if not row:
        await callback.answer("Channel not found",show_alert=True); return
    editing[callback.from_user.id]=f"edit_channel:{rid}"
    await callback.answer()
...
async def admin_input(message: Message, bot: Bot):
    ...
    # Handle normal user's UPI submission before admin-only controls.
    if uid in editing and editing.get(uid) == "reward_upi" and not is_admin(uid):
        key=editing.pop(uid)
    ...
    if uid in editing:
        ...
        if key.startswith("edit_channel:"):
            parts=(message.text or "").splitlines()
            if len(parts)<3:
                await message.answer("❌ Send 3 lines: Channel ID, Channel Name, Join URL."); editing[uid]=key; return
            try: cid=int(parts[0].strip())
            ...
            rid=int(key.split(":")[1])
            try:
                update_channel(rid,cid,parts[1].strip(),parts[2].strip())
                await message.answer("✅ Channel updated. All users must join the updated channel.")
            except sqlite3.IntegrityError:
                await message.answer("❌ That Channel ID is already added. Use a different channel ID."); editing[uid]=key; return
            return
        if key=="new_button":
            parts=(message.text or "").splitlines()
